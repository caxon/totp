...
alias fasrc="{totp_project_dir}/scripts/start-ssh && ssh {LOGIN_HOST_ALIAS}"
...
def make_rc_file(rc_file=RC_FILE):
    rc_file_full = Path(rc_file).expanduser()
    try:
        rc_file_full.touch(exist_ok=False)
        logging.info(f"Empty .rc file created at: {rc_file_full}")
    except FileExistsError:
        raise Exception(
            f"Shell config file ({rc_file_full}) already exists. This should not happen!"
        )
    except Exception as e:
        logging.error(f"Unexpected error creating rc_file. Error={e}")
        return False
    return True
